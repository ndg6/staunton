...
// Proof-of-concept API-reference helper: render ONE function's reference straight
// from its source docstring via `tidy` (signature + each parameter's type and
// default). `file` is root-absolute; `name` picks the single function out of that
// module. Curated lists (not whole modules) are shown, so private `_` helpers
// never appear.
#let show-fn(file, name) = {
  let m = tidy.parse-module(read(file), old-syntax: true)
  m.functions = m.functions.filter(f => f.name == name)
  tidy.show-module(m, first-heading-level: 3, show-outline: false, sort-functions: none)
}
...
== Function reference — `tidy` preview

*Proof of concept.* The three entries below are generated by the `tidy` package
straight from each function's source docstring: the signature first, then every
parameter with its type and default. If we adopt this, the hand-written signature
blocks in _Functions_ (below) are replaced by generated entries like these, split
into *Main functions* and *Behind the scenes* via curated lists.

#[
  #set heading(numbering: none)
  #show-fn("/lib.typ", "board")
  #show-fn("/src/pgn.typ", "parse-pgn")
  #show-fn("/src/game.typ", "with-variation")
]

...
// Standard 8x8 file letters / rank digits, kept for the (still 8x8) renderer and
// any caller that wants the classic set.
...
/// Parse an algebraic square name into zero-based indices, bounds-checked
/// against the board geometry (`cols` x `rows`, default standard 8x8).
/// One leading file letter (a..z) plus a 1+ digit rank. Capitalisation does not
/// matter: "E4" and "e4" are equivalent.  -> (col: int, row: int)
...
/// Inverse of `parse-square`: (col, row) -> "e4". Geometry-agnostic (the indices
/// already encode the position); validates only against the 26-file alphabet.

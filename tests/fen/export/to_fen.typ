...
// string (inverse of position()). Standard positions round-trip exactly.
#import "/lib.typ": to-fen, position, play, game, starting-fen

#set page(width: auto, height: auto, margin: 1cm)
#set text(font: "Libertinus Serif", size: 10pt)

// round-trip: position() -> to-fen is the identity for standard FENs
#let fens = (
  starting-fen,
...
#for f in fens {
  assert(to-fen(position(f)) == f, message: "round-trip failed: " + to-fen(position(f)) + " != " + f)
}

// Strict X-FEN e.p.: an e.p. target with no pawn able to capture it is dropped to
// "-" on output (here c6 has no white pawn on b5/d5), even though position() keeps
// it leniently on input.
#assert(
  to-fen(position("rnbqkbnr/pp1ppppp/8/2p5/4P3/8/PPPP1PPP/RNBQKBNR w KQkq c6 0 2"))
